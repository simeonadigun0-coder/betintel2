// ============================================================
//  BetIntel — get-fixtures.js (v4 — lean & fast)
//  Just fetches team names + kickoff times. No standings.
//  No form. Completes in under 10 seconds.
//  Groq fills in form/standings from its own knowledge.
// ============================================================
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Fetch ALL leagues in one parallel blast — just scoreboard, no enrichment
    const results = await Promise.all(
      LEAGUES.map(async league => {
        try {
          ...
          return data.events
            .filter(e => e.status?.type?.state !== "post" && e.status?.type?.state !== "in")
            .map(e => {
