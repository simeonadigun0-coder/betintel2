...
const handler = async (req, res) => {
  ...
  try {
    // Try today AND tomorrow — covers timezone gaps and late-night fixtures
    const dates = [];
    ...
    const allResults = await Promise.all(
      LEAGUES.map(async league => {
        // Try each date, return first one that has fixtures
    ...
    const fixtures    = allResults.flat().sort((a,b) => a.kickoff.localeCompare(b.kickoff));
