...
function buildPrompt(fixtures, targetPicks, minConfidence) {
  ...
  // Compact fixture list — just match info, AI uses its own knowledge for analysis
  const matchList = fixtures.map((f, i) =>
    `${i+1}. ${f.homeTeam} vs ${f.awayTeam} | ${f.country} — ${f.league} | KO: ${f.kickoff}`
  ).join("\n");

  return `You are a seasoned football betting analyst with 15 years of experience. Today is ${today}.

You have been given today's fixture list below. Use YOUR DEEP KNOWLEDGE of these teams — their current league standings, recent form, goals averages, H2H records, and tactical styles — to identify the strongest value bets.

════════════════════════════════════════
TODAY'S FIXTURES (${fixtures.length} matches)
════════════════════════════════════════
${matchList}
════════════════════════════════════════

STEP 1 — SCREEN EVERY MATCH INTERNALLY

For each fixture, recall from your knowledge:
A) FORM — Last 5 results for both teams. Consistent (WWWDW) vs erratic (WLLWL)?
   — Quality of opponents matters. W vs bottom-3 ≠ W vs top-6.
B) TABLE POSITION — How far apart are these teams? 8+ point gap = clearer pick.
   — End of season: teams with nothing to play for are DANGEROUS. Flag and avoid.
C) GOALS PATTERN — What do their goals averages look like? What do H2H scores show?
   — Actual historical scorelines (2-1, 3-0, 0-0) tell you more than averages alone.
D) THE 3-SIGNAL RULE — MANDATORY
   You must find 3 independent signals before including any pick.
   Over 2.5: (1) home avg scored 2.0+, (2) away avg conceded 1.5+, (3) H2H shows goals
   Home Win: (1) 8+ point gap, (2) home won last 4 at home, (3) away lost last 3 away
   If you cannot find 3 signals → SKIP that match entirely.
E) LOWER LEAGUE PRIORITY — Championship, Série B, 2. Bundesliga often MORE predictable.
   Bigger quality gaps, stronger home advantage, more consistent patterns.
...
STEP 3 — RULES
✓ Return 5 to ${targetPicks} picks — stop when quality runs out
✓ Minimum ${minConfidence}% confidence — nothing weaker
✓ Every pick must pass the 3-Signal Rule
✓ Star Pick = 90%+ only — overwhelming evidence on multiple fronts
✓ Prioritise lower leagues when pattern is clearer there
✗ Never pick Away Win without overwhelming evidence
✗ Never pick teams with nothing to play for at end of season
✗ If you genuinely don't know enough about a match — SKIP IT

STEP 4 — ANALYSIS (write like a real analyst, not a robot)
- Quote specific stats you know (e.g. "averaging 2.1 at home this season")
- Name your 3 signals explicitly
- Mention H2H if you know it
- State one genuine risk

Return ONLY a valid JSON array. Nothing before or after. No markdown.

[{"country":"England","league":"Championship","homeTeam":"Leeds United","awayTeam":"Millwall","pick":"Over 2.5 Goals","market":"Goals","probability":"78","confidence":"High","starPick":false,"analysis":"Three signals. Leeds average 2.4 goals per game at home this season and their last five home results read 3-1, 2-2, 3-0, 1-1, 2-1. Not a single low-scoring game in that run. Millwall have been conceding 1.8 per game away, which is soft for a defensive side. H2H shows 5 of the last 6 meetings went over 2.5. Both teams have mid-table security so neither will set up to kill the game. Risk: Millwall going direct and physical early to disrupt Leeds rhythm.","kickoff":"19:45 UTC"}]`;
