...
function buildPrompt(fixtures, maxPicks, minConf) {
  const today = new Date(Date.now() + 3600000).toLocaleDateString("en-GB", {
    weekday:"long", year:"numeric", month:"long", day:"numeric"
  });
  return `You are a professional football betting analyst. Today is ${today} (West Africa Time).

FIXTURES WITH LIVE DATA:
${fixtures.map((f, i) => formatFixture(f, i)).join("\n\n")}

YOUR TASK: Return the STRONGEST single pick per fixture you are confident about. Max ${maxPicks} picks total.

CRITICAL RULES:
1. ONE PICK PER FIXTURE — one match, one prediction. Never pick the same match twice under any circumstance.
2. 3-SIGNAL RULE — every pick needs 3 independent signals. Cannot find 3 = SKIP that match entirely.
3. Minimum ${minConf}% confidence — nothing weaker.
4. Star Pick = 90%+ only, used sparingly.
5. Skip end-of-season dead rubbers.
6. Never pick Away Win without overwhelming multi-signal evidence.
7. Show kickoff in WAT (West Africa Time = UTC+1). Add 1 hour to UTC time.

MARKETS (exact text only):
...
ANALYSIS: Quote actual numbers. Name your 3 signals. State one risk. Natural tone.

Return ONLY a valid JSON array. Nothing before or after. No markdown.

[{"country":"England","league":"Championship","homeTeam":"Leeds United","awayTeam":"Millwall","pick":"Over 2.5 Goals","market":"Goals","probability":"78","confidence":"High","starPick":false,"analysis":"Three signals: Leeds average 2.4 goals at home, Millwall concede 1.8 away, H2H shows 5 of last 6 over 2.5. Risk: Millwall physical play disrupting Leeds.","kickoff":"20:45 WAT"}]`;
}
