// ============================================================
//  BetIntel — analyze.js (v7)
//  Single Groq call — 8 fixtures — guaranteed under 30s
//  Vercel body parser explicitly enabled
// ============================================================
...
Find up to ${maxPicks} value bets from these fixtures. Use the live data. Fill gaps with your knowledge.

3-SIGNAL RULE: Every pick needs 3 independent signals or skip it.

MARKETS (use exact text):
...
RULES: Min ${minConf}% confidence. Star Pick=90%+ only. Skip dead rubbers. Skip Away Win without strong evidence.

Write analysis naturally — quote actual numbers, name 3 signals, state one risk.
...
module.exports = async (req, res) => {
  ...
  try {
    // Safely parse body — handles string, object, or undefined
    let body = {};
    if (req.body) {
    ...
    // Use top 8 enriched fixtures — single Groq call, ~20s, well under 60s limit
    const sorted = [...fixtures].sort((a, b) => {
      const aScore = Object.keys(a.homeStanding || {}).length + Object.keys(a.homeForm || {}).length;
      const bScore = Object.keys(b.homeStanding || {}).length + Object.keys(b.homeForm || {}).length;
      return bScore - aScore; // richest data first
    });
    const toAnalyse = sorted.slice(0, 8);

    if (!toAnalyse.length) {
      return res.status(400).json({ error: "No fixtures available to analyse." });
    }
    ...
    if (!groqRes.ok) {
      ...
      throw new Error(`Groq HTTP ${groqRes.status}: ${errText.substring(0, 100)}`);
    }
    ...
    const rawPicks = JSON.parse(match[0]);
